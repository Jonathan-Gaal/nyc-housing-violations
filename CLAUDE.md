...
Full context: `context/00_START_HERE.md` (architecture), `context/API_INTEGRATION.md` (Socrata query shape), `context/technologies/user-stories/user_stories.md` (US-1..US-10), `context/technologies/snippets/schemas/schema_firebase.sql` (DB schema), `context/technologies/snippets/snippets.md` (code library), `context/data-context/*` (CSV verification).
...
| `jazz` | Yes | Reads this context tree once per session so shockwave/wheeljack don't re-read it. |
...
| `wheeljack` | Yes | Primary builder — data loader, API routes, rating calc, React components, Mapbox heatmap. |
...
- **Bounded-AI boundary**: the building rating score (violation count / age / rent-impairing weights → 0–5 stars, per US-5 in `user_stories.md`) is computed **deterministically in code** (loader or API route). No LLM ever produces the score or the ranking.
...
- Local wiki: `../../../../.claude/wiki/project-nyc-open-data-project/`
- Materialized agents: `.claude/agents/` (this project)
